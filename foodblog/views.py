...
from .forms import CommentForm, ContactForm, SearchForm, SubscribeForm, UnsubscribeForm
...
def getComments(post_id, page):
    items_per_page = 10
    try:
        comments = Comment.objects.filter(post=post_id, status="a").order_by(
            "-pub_date"
        )
...
def index(req, page=1):  # default number for page is 1
    ...
    if req.method == "POST":
        ...
        if form.is_valid():
            ...
            if Subscriber.objects.create(
                sub_email=form.cleaned_data["subscribe_email"]
            ):
...
def thanks(req, forwhat="subscibe"):
    messages = {
        "subscribe": {
            ...
            "message": gettext(
                "Thanks for subscribing our blog.<br>You will receive our latest post ASAP!"
            ),
        },
        "unsubscribe": {
    ...
    if req.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                return HttpResponseRedirect(
                    reverse("foodblog:thanks", args=["unsubscribe"])
                )

            except Subscriber.DoesNotExist:
                return HttpResponseRedirect(
                    reverse("foodblog:thanks", args=["notsubscribed"])
                )
        ...
        if form.is_valid():
            ...
            if captcha_result["success"]:
                ...
                if results > 0:
                    ...
                    messages.error(
                        req, gettext("Message sent failed! Please try again later!")
                    )
            else:
                messages.error(
                    req, gettext("reCaptcha is failed! You cannot  send message.")
                )
    else:
        pass
